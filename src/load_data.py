...
class APTOS(torch.utils.data.Dataset):
    def __init__(self, path, transform=None, split='train', seed=0):
        self.transform = transform
        self.data = []  

        if split == 'train':
            normal_dir = os.path.join(path, "APTOS", "train", "NORMAL")
            for img_file in os.listdir(normal_dir):
                self.data.append((os.path.join(normal_dir, img_file), 0))
        else:
            # normal test samples
            normal_dir = os.path.join(path, "APTOS", "test", "NORMAL")
            for img_file in os.listdir(normal_dir):
                self.data.append((os.path.join(normal_dir, img_file), 0))

            # abnormal test samples
            abnormal_dir = os.path.join(path, "APTOS", "test", "ABNORMAL")
            for img_file in os.listdir(abnormal_dir):
                self.data.append((os.path.join(abnormal_dir, img_file), 1))
...
def prepare_loader(image_size, path, dataset_name, class_name, batch_size, test_batch_size, num_workers, seed, shots):
    ...
    if dataset_name == 'mvtec-ad-loco-ad' or dataset_name == 'mvtec-ad-ad' or dataset_name == 'mpdd':
        ...
        train_set = APTOS(path, transform=transform, split='train')
        test_set  = APTOS(path, transform=transform, split='test')
    else:
